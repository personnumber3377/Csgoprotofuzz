		...
		if message.msg_type in usermessage_types:
			...
			if not usermessage_types[message.msg_type] == "CCSUsrMsg_UpdateTeamMoney":
				...
				if not skip:
					print("User message: ")
					print(message)
					print("Here is the message contents:")
					message2, mutated = mutate_user_message(message2)
...
def fuzz(buf, add_buf, max_size, wanted_type=False, out_dir=None):
...
try:
	...
	print("message_type: "+str(message_type))
	print("wanted_type: "+str(wanted_type))
	#print("proto.svc_EntityMessage == "+str(proto.svc_EntityMessage))
	result_thing = False

	if message_type == wanted_type:
		print("This message is the wanted type!")
		result_thing = True
	...
	#print("poopoo")
	#print(message)
	print("type == "+str(message_type))
	print("Message is this: "+str(message))
	print("Type of message is this: "+str(type(message)))
	if message_type == "CSVCMsg_GameEventList":
		print("Descriptors:")
		print(message.descriptors)
		print("len(message.descriptors) == "+str(len(message.descriptors)))
		for desc in message.descriptors:
			print("="*40)
			print(desc)
			print("desc.name == "+str(desc.name))
			print("="*40)
			if desc.name == "smokegrenade_detonate":
				print("Detonation")
				
				#print("entity id thing: "+str(desc.entityid))
	return result_thing
...
if __name__=="__main__":
	...
	if "--indir" not in sys.argv:
		print("Pass input dir with --indir DIRECTORY .")
		exit(1)


	if "--outdir" not in sys.argv:
		print("Pass output dir with --outdir DIRECTORY .")
		exit(1)
	...
	wanted_type = str(input("Which type of message would you like to filter out? : "))
	...
	for file in files:
		...
		if not hasattr(proto, wanted_type):
			print("The message type is not valid!")
			exit(1)
		thing = fuzz(bytes_buffer, 0, 10000, wanted_type)

		if thing:
			print("Copying file " + str(file) + " to "+str(output_dir))
			os.system("cp "+str(indir)+str(file)+" "+str(output_dir)+str(file))
