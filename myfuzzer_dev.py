...
SMALL_INT_CHANCE = 0.5
OWN_MUTATOR_CHANCE = 0.3

BYTES_MUTATION_CHANCE = 0.999
...
def set_small_int(message, field):
	random_shit = random.randrange(SMALL_INT_MIN, SMALL_INT_MAX)
...
def mutate_submessage(message):
	...
	if field_type == TYPE_INT32:
		...
		if random.random() < SMALL_INT_CHANCE:

			message, _ = set_small_int(message,random_field)
...
def fuzz(buf, add_buf, max_size):
	if AS_MAIN:
		error_log("AS_MAIN is set to True. This should only be used in debugging and developing of the mutator. This shouldn't appear when actually fuzzing.")
...
try:
	...
	if AS_MAIN:
		...
		original_messagething = copy.deepcopy(message)
	...
	if AS_MAIN:
		...
		same_thing = compare_messages(original_messagething, message)
		if same_thing:
			print("="*50)
			
			print("Mutated: "+str(mutated))
			print("Mutated message is the same as original message! This shouldn't happen!")
			print("Original message: "+str(original_messagething))
			print("New message: "+str(message))
			
			print("type(message) == "+str(type(message)))
			print("="*50)
			#if "UserMessage" in str(type(message)):
			#	faileduser += 1
