...
...     print("Generated object: {}".format(obj))
	...
	#return list(fuzzed_stuff)[random.randrange(len(list(fuzzed_stuff)))]

	#return fuzzed_stuff[random.randrange(len(fuzzed_stuff))]
...
def fuzz(buf, add_buf, max_size, filename=False):
	...
	print("Printing message type: ")
	
	if message_type == "CSVCMsg_GameEventList":
		
		if filename != False:
			print("Command: "+str("cp "+str(filename)+" eventpackets/"))
			os.system("cp "+str(filename)+" eventpackets/")
...
if __name__=="__main__":
	...
	for file in test_files:
		...
		stuff = fuzz(bytes_buffer, 0, 0, filename)
