...
try:
	...
	if len(bytes(type_bytes)+bytes(len_bytes)+mut_msg_bytes) > max_size:
		if (len(mut_msg_bytes)) != parse_length(len_bytes)[0]:
			print("Shit!")
			exit(1)
		return bytearray(original_message)
	else:
		#print("Returning modified thing")
		#print("Length: "+str(len(bytes(type_bytes)+bytes(len_bytes)+mut_msg_bytes)))
		#print(type(mut_msg_bytes))
		if (len(mut_msg_bytes)) != parse_length(len_bytes)[0]:
			print("Shit!")
			print("")
			exit(1)
		...
		#print(type(mut_msg_bytes))
		#return mut_msg_bytes
		#return bytes(type_bytes)+bytes(len_bytes)+mut_msg_bytes  # return the packet
		#buf = mut_msg_bytes
